    ...
    private static class sendMsg extends AbstractFunction1<EdgeTriplet<Payload, Integer>, Iterator<Tuple2<Object, Payload>>> implements Serializable {
        ...
        public Iterator<Tuple2<Object, Payload>> apply(EdgeTriplet<Payload, Integer> triplet) {
            ...
            if (sourceVertex._2.getValue() == Integer.MAX_VALUE || sourceVertex._2.getValue() + distance >= dstVertex._2.getValue()) {
                ...
                Payload newPayload = new Payload(sourceVertex._2.getValue() + distance);
                newPayload.setVertices(sourceVertex._2.getVertices());
                newPayload.addVertex((Long) dstVertex._1);
                return JavaConverters.asScalaIteratorConverter(Arrays.asList(new Tuple2<Object, Payload>(triplet.dstId(), newPayload)).iterator()).asScala();
            }
        }
    }
